...
class VFH_Class : public ThreadedDriver
{
  ...
  private:
    ...
    int SetupRanger();
    int ShutdownRanger();
    void ProcessLaser(const player_laser_data_t &);
    void ProcessSonar(const player_sonar_data_t &);
    void ProcessRanger(const player_ranger_data_range_t &);
...
void VFH_Class::DoOneUpdate()
{
  ...
  const double angdiff = this->angle_diff((double)this->goal_t,this->odom_pose[2]);
  ...
  //         stop.
          ...
          (std::abs(DTOR(angdiff)) < this->ang_eps))
  {
    this->active_goal = false;
...
//        int foo = this->speed;
  ...
  else
  {
    ...
    turnrate = (int)rint(std::abs(angdiff/180.0) *
...
{
  const int x = (int)rint(cmd.pos.px * 1e3);
  const int y = (int)rint(cmd.pos.py * 1e3);
  const int t = (int)rint(RTOD(cmd.pos.pa));
...
{
  const double ra = NORMALIZE(DTOR(a));
  const double rb = NORMALIZE(DTOR(b));

  const double d1 = ra-rb;
  double d2 = 2.0*M_PI - std::abs(d1);
  if(d1 > 0.0)
    d2 *= -1.0;
